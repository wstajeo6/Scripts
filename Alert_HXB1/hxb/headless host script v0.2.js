...
const token = "thr1.AAAAAF-trr6c89uftVa8pg.hpg15Qdyibs";
...
var adminPassword = "!plgdad07";//1000 + getRandomInt(9000);
...
var commands = {
	"help": {
		...
		"roles": Role.PLAYER,
		"desc": `
			Komenda wyświetlająca informacje o wszystkich dostępnych komendach lub informacje o konkretnej komendzie.
			Przykład: \'!help bb\' wyświetli opis komendy \'bb\'.`,
	...
	"bb": {
		...
		"roles": Role.PLAYER,
		"desc": `
			Komenda, która wykopie cię z pokoju na zbity pysk. (zalecany sposób opuszczenia pokoju).`,
...
function leaveCommand(player, message) {
	room.kickPlayer(player.id, "Nara! Ale wróć jeszcze.", false);
}
...
function helpCommand(player, message) {
	...
	if (msgArray.length === 0) {
		...
		if (commandString.slice(commandString.length - 1) === ":") commandString += ` None,`;
		commandString = commandString.substring(0, commandString.length - 1) + ".";
		commandString += "\n\n Aby uzyskać informacje o danej komendzie, wpisz '\'!help <nazwa_komendy_bez_wykrzyknika>\'.";
		room.sendAnnouncement(commandString, player.id, statsColor, "bold", Notification.CHAT);
	}
	else if (msgArray.length >= 1) {
		var commandName = getCommand(msgArray[0].toLowerCase());
		if (commandName !== false && commands[commandName].desc !== false) room.sendAnnouncement(`\'${commandName}\' komenda:\n${commands[commandName].desc}`, player.id, statsColor, "bold", Notification.CHAT);
		else room.sendAnnouncement(`Nie znaleziono komendy. Aby sprawdzić dostępne komendy, wpisz \'!help\'.`, player.id, statsColor, "bold", Notification.CHAT);
	}
}
...
function adminCommand(player, message) {
	...
	if (parseInt(msgArray[0]) === adminPassword) {
		...
		room.sendAnnouncement(`${player.name} jest od teraz room masterem!`, null, announcementColor, "bold", Notification.CHAT);
	}
}
...
function checkTime() {
	...
	if (Math.abs(scores.time - scores.timeLimit) <= 0.01 && scores.timeLimit !== 0 && activePlay) {
		...
		goldenGoal = true;
		room.sendAnnouncement("⚽ Złoty gol wygrywa!", null, announcementColor, "bold", Notification.CHAT);
	}
	if (Math.abs(drawTimeLimit * 60 - scores.time - 60) <= 0.01 && players.length > 2) {
		if (checkTimeVariable === false) {
			...
			if (activePlay) room.sendAnnouncement("⌛ 60 sekund do końca dogrywki!", null, announcementColor, "bold", Notification.CHAT);
		}
	}
...
function endGame(winner) { // no stopGame function in it
	const scores = room.getScores();
	if (winner === Team.RED) {
		room.sendAnnouncement(`🏆 Red Team wygrywa ${scores.red} - ${scores.blue}!`, null, redColor, "bold", Notification.CHAT);
	}
	else if (winner === Team.BLUE) {
		room.sendAnnouncement(`🏆 Blue Team wygrywa ${scores.blue} - ${scores.red}!`, null, blueColor, "bold", Notification.CHAT);
	}
	else {
		room.sendAnnouncement("🕑 Koniec dogrywki. Remis!", null, announcementColor, "bold", Notification.CHAT);
	}
}
...
room.onPlayerJoin = function (player) {
	room.sendAnnouncement(`👋 Witaj ${player.name}!`, player.id, welcomeColor, "bold", Notification.CHAT);
	updateTeams();
	updateAdmins();
	...
	if (lastPlayersTouched[0] !== null) {
		if (lastPlayersTouched[0].team === team) {
			if (lastPlayersTouched[1] !== null && lastPlayersTouched[1].team === team) {
				room.sendAnnouncement(`⚽ ${getTime(scores)} Gol: ${lastPlayersTouched[0].name}! Asysta: ${lastPlayersTouched[1].name}. Prędkość: ${ballSpeed.toFixed(2)}km/h.`, "bold", (team === Team.RED ? redColor : blueColor), null, Notification.CHAT);
			}
			else {
				room.sendAnnouncement(`⚽ ${getTime(scores)} Gol: ${lastPlayersTouched[0].name}! Prędkość: ${ballSpeed.toFixed(2)}km/h.`, null, (team === Team.RED ? redColor : blueColor), "bold", Notification.CHAT);
			}
		}
		else {
			room.sendAnnouncement(`⚽ ${getTime(scores)} Bramka samobójcza: ${lastPlayersTouched[0].name}! Prędkość: ${ballSpeed.toFixed(2)}km/h.`, null, (team === Team.RED ? redColor : blueColor), "bold", Notification.CHAT);
		}
	}
